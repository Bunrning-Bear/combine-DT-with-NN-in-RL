...
class MultiSession(object):
    ...
    def make_session(self, graph, num_cpu):
    ...
    def eval(self, expr, feed_dict=None):
        if feed_dict is None:
            feed_dict = {}
        return self.sess.run(expr, feed_dict=feed_dict)

    def set_value(self, v, val):
        if v in self.VALUE_SETTERS:
            set_op, set_endpoint = self.VALUE_SETTERS[v]
        else:
...
# ================================================================
    ...
    def function(self, inputs, outputs, updates=None, givens=None):
        ...
        if isinstance(outputs, list):
            return _Function(self.sess, inputs, outputs, updates, givens=givens)
        elif isinstance(outputs, (dict, collections.OrderedDict)):
            f = _Function(self.sess, inputs, outputs.values(), updates, givens=givens)
            return lambda *args, **kwargs: type(outputs)(zip(outputs.keys(), f(*args, **kwargs)))
        else:
            f = _Function(self.sess, inputs, [outputs], updates, givens=givens)
            return lambda *args, **kwargs: f(*args, **kwargs)[0]


class _Function(object):
    def __init__(self, session, inputs, outputs, updates, givens, check_nan=False):
        for inpt in inputs:
    ...
    def __call__(self, *args, **kwargs):
        ...
        results = self.session.run(self.outputs_update, feed_dict=feed_dict)[:-1]
        if self.check_nan:
