...
- 2017-8-13
  ...
  - 第一条被证明是概率事件，prioritized replay buffer 被证明没有取得相当效果

- 2017-8-15：

  - 转移实验环境，使用cartpole-v1进行实验，使用普通的replay buffer，在全分布条件下取得了相当的效果。
  - 但是进行真正的决策树分布时，取得了比较糟糕的效果，4个节点的时候，出现了reward到达20这样非常低的水平的情况。2个节点也出现了训练速度的下降
  - 检查：
    - 单个节点是否有问题:没问题
    - 两个节点分布的部分，进行单步调试做一次逻辑上的检查，看看是否有问题。
    - 每一个状态进行更多个属性的分割，查看效果：效果不好
  - 做几个维度的对比实验：
    - 随着网络深度的增加，相同的分割节点表现如何
    - 相同的网络深度，各个分割节点表现如何：2,10,100。1 layer
    - 算法正确性检验。

- 2017-8-17：

  - splite-1：多做几次重复实验，他是真高还是假高。--- 这是一个概率事件
  - splite，随着forest的增加
  - 用 2layer 试试splite是不是相同的影响 
  - 相同的splite，不同的层次有什么影响：
    - 使用splite-1，不同的depth，发现这样的问题：
      - depth=1，也就是两个节点的时候，他在原来实验的2倍时间开始有起色。
      - depth
  - 测试是不是随机性增加了：对比原本的baseline对不同次数的实验的区别
  - 贴上全分布和0层的对比图，说明算法基本正确。 --ok
  - ​
